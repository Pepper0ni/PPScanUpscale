...
imageMed = np.median(hsvImage, (0, 1))
electric = False
if 20 < imageMed[0] < 30 and imageMed[1] > 100:
    electric = True
print(electric)
...
while True:
    ...
    if electric:
        borderBase = cv.vconcat([trimImage(hsvImage, START, STOP, START, hsvImage.shape[1]-START), trimImage(hsvImage, hsvImage.shape[0]-STOP, hsvImage.shape[0]-START, START, hsvImage.shape[1]-START)])

        borderBaseV = cv.hconcat([trimImage(hsvImage, START, hsvImage.shape[0]-START, START, STOP),trimImage(hsvImage, START, hsvImage.shape[0]-START, hsvImage.shape[1]-STOP, hsvImage.shape[1]-START)])
        ...
        frame_threshold = cv.inRange(hsvImage, (int(lowPerc[0] - low_H), int(lowPerc[1] - low_S), int(lowPerc[2] - low_V)),
                                 (int((highPerc[0] + high_H)), int(highPerc[1] + high_S), int(highPerc[2] + high_V)))
    else:
        frame_threshold = cv.inRange(hsvImage,(int(21 - low_H), int(183 - low_S), int(203 - low_V)),
                                     (int((29 + high_H)), int(252 + high_S), int(252 + high_V)))
