...
def CustomBordersUI(src): #open a UI that allows you to set custom border positions on an image
    global declared
    cv.namedWindow(TRACKBAR_WINDOW_NAME)
    if not declared:
        posDict = {"TLX": 0, "TLY": 0, "TMX": round(src.shape[1] / 2), "TMY": 0, "TRX": src.shape[1],
                                "TRY": 0, "MLX": 0,
                                "MLY": round(src.shape[0] / 2), "MRX": src.shape[1], "MRY": round(src.shape[0] / 2),
                                "BLX": 0,
                                "BLY": src.shape[0], "BMX": round(src.shape[1] / 2), "BMY": src.shape[0],
                                "BRX": src.shape[1], "BRY": src.shape[0]} #set starting position as a dict that can be looped over
